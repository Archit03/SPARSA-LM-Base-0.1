...
class Trainer:
    def __init__(self, training_config: Dict, data_config: Dict):
        try:
            ...
            self.data_config = data_config.get('datasets', [])
            if not isinstance(self.data_config, list):
                raise ConfigurationError("'datasets' in data_config must be a list of dataset definitions.")

            # Extract train and val dataset configs
            train_config = next((d for d in self.data_config if d['name'] == self.config['dataset']['train_dataset']), None)
            val_config = next((d for d in self.data_config if d['name'] == self.config['dataset']['val_dataset']), None)

            if not train_config or not val_config:
                raise ConfigurationError("Train or validation dataset configuration is missing.")
            ...
            self.dataset_processor = DatasetProcessor(train_config, val_config, preprocessing_config)
